...
`gdsNodes()` is an exported function. It takes the GDS file path as
input, and returns all nodes that can be converted to `GDSArray`
instances. The returned GDS node names can be used as input for the
...
returns the object with rows being "genes / variants / snps" and the
columns being "samples". This is consistent with the assay data inside
