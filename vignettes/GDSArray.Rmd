...
The `GDSFile` is a light-weight class of representing GDS files. It
...
`current_path` in `GDSFile` object represent a valid gds
node. Otherwise, it will return the `GDSFile` object with an updated `current_path`.  
...
Try type in `gf$ann` and press `tab` key for the completion.  
